...
describe("tilt", () => {
  it("cursor right biases root.rotation.y positive", () => {
    ...
    const ctx = {
      bodies: [], input: { cursorWorld: new Vector3(0.5, 0, 0), justClicked: false, lastClickWorld: null },
  ...
  it("no cursor eases back toward zero", () => {
    ...
    root.rotation.y = 0.5;
    const ctx = {
      bodies: [], input: { cursorWorld: null, justClicked: false, lastClickWorld: null },
      root, dt: 1 / 60, voxelSize: 0.05,
    };
  ...
  it("force() is zero", () => {
    ...
    const f = tiltMode.force({ home: new Vector3(), pos: new Vector3(), vel: new Vector3(), rot: new Vector3(), angVel: new Vector3() },
      { bodies: [], input: { cursorWorld: null, justClicked: false, lastClickWorld: null }, root, dt: 1 / 60, voxelSize: 0.05 });
