...
1. **Images are not copied.** `public/res` is a symlink to the docs checkout (`CONTENT_PATH`), so each workshop's `images/` folder is served verbatim at `/res/<workshop>/images/` and Astro dereferences the symlink into `dist/` at build time.
...
- **`rehype-markdown.ts`** holds three small rehype plugins: `rehypeHeadingIds` forces a `slugify()` id on every h1–h6 (overriding Astro's github-slugger, so all ids come from the single slugger); `rehypeWrapTables` wraps `<table>` for horizontal scroll; `rehypeImagePaths` rewrites `images/foo.png` → `/res/<workshopDir>/images/foo.png` (workshop dir = last two segments of the entry path), treats a numeric `title` as a max-width, and promotes lone-image paragraphs into `<figure class="md-figure">` with `alt` as caption (unless `alt` looks like a filename).
